...
from src.core.config import (
    OIDC_BASE,
    OIDC_REGISTER_URL as REGISTER_URL,
    OIDC_DEVICE_AUTH_URL as DEVICE_AUTH_URL,
    OIDC_TOKEN_URL as TOKEN_URL,
    OIDC_START_URL as START_URL,
)
...
async def register_client_min() -> Tuple[str, str, Optional[str]]:
    """
    Register an OIDC client (minimal) and return (clientId, clientSecret, registrationExpiresAt).
    """
    payload = {
        "clientName": "Kiro IDE",
        "clientType": "public",
    ...
    async with httpx.AsyncClient(mounts=mounts, timeout=timeout) as client:
        ...
        r.raise_for_status()
        data = r.json()
        # clientSecretExpiresAt 是 Unix 时间戳（秒），转换为 ISO 格式
        registration_expires_at = None
        if data.get("clientSecretExpiresAt"):
            try:
                ts = int(data["clientSecretExpiresAt"])
                registration_expires_at = time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime(ts))
            except (ValueError, TypeError):
                pass
        return data["clientId"], data["clientSecret"], registration_expires_at
...
async def device_authorize(client_id: str, client_secret: str, start_url: Optional[str] = None) -> Dict:
    ...
    Args:
        ...
        start_url: Builder ID Start URL (支持 AWS IAM Identity Center，默认使用标准 AWS Builder ID)
    """
    payload = {
        "clientId": client_id,
        "clientSecret": client_secret,
        "startUrl": start_url or START_URL,
    }
    proxies = _get_proxies()
    mounts = None
    if proxies:
