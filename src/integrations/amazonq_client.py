...
from src.core.delta_utils import delta_by_prefix as _delta_by_prefix
...
@dataclass
class ChatResponse:
    """send_chat_request 的返回值封装"""
    text: Optional[str]  # 非流式响应的完整文本
    text_stream: Optional[AsyncGenerator[str, None]]  # 流式文本生成器
    tracker: "StreamTracker"  # 流跟踪器
    event_stream: Optional[AsyncGenerator[Any, None]]  # 原始事件流生成器
...
def _get_proxies() -> Optional[Dict[str, str]]:
    """获取代理配置，支持 HTTP_PROXY, HTTPS_PROXY"""
...
async def _dedupe_assistant_content_events(
    ...
    """
    去重 assistantResponseEvent 事件，跳过连续完全相同的 content

    参考 AIClient-2-API 的 Kiro 去重逻辑：
    - 只比较连续的两个 content 事件是否完全相同
    - 不使用 delta 计算，避免状态不一致
    - Kiro/Amazon Q API 有时会重复发送相同的内容事件
    """
    last_content = None
    async for event_type, payload in events:
        if event_type == "assistantResponseEvent" and isinstance(payload, dict):
            content = payload.get("content")
            # 检查是否与上一个 content 事件完全相同（包括 None 和空字符串）
            if content is not None and content == last_content:
                # 跳过重复的内容（Kiro API 有时会重复发送）
                logger.debug("Dedupe: skipped duplicate content: %s...", str(content)[:50])
                continue
...
def _sanitize_error_message(err: str, status_code: int) -> str:
    """脱敏错误信息, 移除可能的敏感内容"""
    if _is_debug_mode():
        return err
    # 非 DEBUG 模式下, 对未知错误进行脱敏
    # 已知的友好错误消息直接返回
    known_messages = [
        "输入内容超过上游API限制",
        "账号月度配额已耗尽",
...
async def _handle_error_response(
    ...
    if resp.status_code >= 500:
        upstream_request_id = resp.headers.get("x-amzn-requestid") or resp.headers.get("x-amzn-request-id")
        upstream_error_type = resp.headers.get("x-amzn-errortype") or resp.headers.get("x-amzn-error-type")
        preview = (check_err or "").replace("\n", " ").replace("\r", " ")[:500]
        logger.error(
            "[Upstream] HTTP %s, request_id=%s, error_type=%s, body=%s",
            resp.status_code,
            upstream_request_id or "-",
            upstream_error_type or "-",
            preview,
        )
    ...
    if resp.status_code in (401, 403):
        raise AccountUnauthorizedException(f"HTTP {resp.status_code}: {err}")
    # 脱敏处理后返回给客户端
    sanitized_err = _sanitize_error_message(err, resp.status_code)
    raise HTTPException(status_code=resp.status_code, detail=sanitized_err)
...
async def send_chat_request(
    ...
    Returns:
        ChatResponse: 包含 text, text_stream, tracker, event_stream 的数据类
    """
    # 1. 准备请求
    if raw_payload:
    ...
    try:
        ...
        async def _iter_text() -> AsyncGenerator[str, None]:
            """
            Extract text content from event stream with deduplication.
            """
            last_text = ""
            async for event_type, parsed in _iter_events():
                text = _extract_text_from_event(parsed)
                if isinstance(text, str) and text:
                    # Apply delta calculation for deduplication
        ...
        if stream:
            ...
            if raw_payload:
                ...
                return ChatResponse(text=None, text_stream=None, tracker=tracker, event_stream=_safe_event_gen())

            # Wrap text stream with aclosing to ensure cleanup
            async def _safe_text_gen():
                try:
                    # raw_payload=False means events need delta calculation
            ...
            return ChatResponse(text=None, text_stream=_safe_text_gen(), tracker=tracker, event_stream=None)
        else:
            buf = []
            try:
                # raw_payload=False means events need delta calculation
                ...
                if not response_consumed and resp:
                    ...
                    if local_client:
                        await client.aclose()
            return ChatResponse(text="".join(buf), text_stream=None, tracker=tracker, event_stream=None)
